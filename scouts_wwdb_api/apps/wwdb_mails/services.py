...
def send_template_mail(*, template: str, from_email: str = None, recipients: list = [], **kwargs):
    subject_message = get_mail_subject_and_message(template=template, **kwargs)

    send_wwdb_mail(
        subject=subject_message[0], html_message=subject_message[1], from_email=from_email, recipients=recipients
    )
